#!/usr/bin/env python3
"""Visualize a trained random-point navigation policy with the PyBullet GUI.

Usage:
    python evaluate_trained_policy_gui.py [model_path] [num_episodes] [max_steps]
...
from train_dqn_random_nav import RandomPointNavEnv
from LLM.llm import Navigator


DEFAULT_MODEL_SESSION_DIR = Path(__file__).resolve().parent / "training_sessions" / "20251209_203306"
...
def test_model(
    model_path: str = str(DEFAULT_MODEL_SESSION_DIR),
    num_episodes: int = 5,
    ...
    if model_path_obj.is_dir():
        candidates = sorted(model_path_obj.glob("final_model_*.zip"), key=lambda p: p.stat().st_mtime, reverse=True)
        if not candidates:
            candidates = sorted(model_path_obj.glob("*.zip"), key=lambda p: p.stat().st_mtime, reverse=True)
        if candidates:
            model_path_obj = candidates[0]
            print(f"Auto-selected model: {model_path_obj}")

    # Stable-Baselines3 accepts either the .zip path or a base path without extension.
    if model_path_obj.suffix.lower() != ".zip":
        zip_candidate = model_path_obj.with_suffix(".zip")
        if zip_candidate.exists():
            model_path_obj = zip_candidate

    try:
        model = DQN.load(str(model_path_obj))
    ...
    # Override LiDAR range for testing/visualization runs.
    # (Training env default is 2.0m; this keeps evaluation experiments isolated.)
    env.lidar_range = float(lidar_range_m)
    if hasattr(env, "scene") and env.scene is not None:
        setattr(env.scene, "lidar_range", float(lidar_range_m))
    print(f"Using LiDAR range: {env.lidar_range:.2f}m")
    ...
    try:
        ...
        if initial_segment and (hasattr(env.scene, 'relocate_humans') or hasattr(env.scene, 'reposition_humans')):
            ...
            if hasattr(env.scene, 'relocate_humans'):
                env.scene.relocate_humans(target_direction.tolist())
            else:
        ...
        for episode in range(1, num_episodes + 1):
            ...
            # Stuck detection
            stuck_counter = 0
            # Detect "real" navigation progress in XY only (robust to up/down bobbing).
            best_xy_dist = float('inf')
            STUCK_THRESHOLD = 5000  # step-based; approximate wall time depends on loop cadence
            PROGRESS_EPS_XY_M = 0.2

            # Time-based fallback: trigger if we have not made meaningful XY progress for N seconds.
            # This is more reliable than step counts when the sim runs faster/slower.
            STUCK_TIMEOUT_SEC = 5.0
            last_progress_time = time.time()

            # Windowed motion detector to catch hovering/oscillation: tiny XY travel for a short window.
            MOTION_WINDOW_SEC = 2.0
            XY_STALL_DIAG_M = 0.25
            recent_positions = deque()  # items: (t_seconds, np.array([x,y,z]))
            ...
            while not (terminated or truncated) and steps < max_steps_per_episode:
                ...
                # --- STUCK DETECTION ---
                # Track motion over a short window (real-time seconds).
                now_t = time.time()
                recent_positions.append((now_t, drone_pos.copy()))
                while recent_positions and (now_t - recent_positions[0][0]) > MOTION_WINDOW_SEC:
                    recent_positions.popleft()

                # Progress metric: XY distance only (ignore Z so bobbing doesn't "unstick" us).
                xy_dist_to_wp = float(np.linalg.norm(drone_pos[:2] - np.array(target_pos[:2])))
                if best_xy_dist == float('inf'):
                    best_xy_dist = xy_dist_to_wp

                made_progress = xy_dist_to_wp < (best_xy_dist - PROGRESS_EPS_XY_M)
                if made_progress:
                    best_xy_dist = xy_dist_to_wp
                    stuck_counter = 0
                ...
                no_progress_sec = now_t - last_progress_time

                # Stall heuristic: very small XY movement over the recent window.
                stall_detected = False
                xy_diag = None
                if len(recent_positions) >= 10:
                    xs = [pos[0] for _, pos in recent_positions]
                    ys = [pos[1] for _, pos in recent_positions]
                    xy_diag = math.hypot(max(xs) - min(xs), max(ys) - min(ys))
                    if xy_diag < XY_STALL_DIAG_M:
                        stall_detected = True

                timeout_detected = no_progress_sec > STUCK_TIMEOUT_SEC

                # If stuck, consult LLM
                if timeout_detected or stall_detected or stuck_counter > STUCK_THRESHOLD:
                    reason = "timeout" if timeout_detected else ("stall" if stall_detected else "steps")
                    extra = f" (xy_diag={xy_diag:.3f}m)" if xy_diag is not None else ""
                    print(f"⚠️ STUCK [{reason}] for {stuck_counter} steps, {no_progress_sec:.1f}s without progress{extra}. Consulting LLM...")
                ...
                ep_reward += reward
                steps += 1
                time.sleep(0.001)  # Faster simulation (1000 Hz instead of 100 Hz)
...
if __name__ == "__main__":
    model_path = sys.argv[1] if len(sys.argv) > 1 else str(DEFAULT_MODEL_SESSION_DIR)
